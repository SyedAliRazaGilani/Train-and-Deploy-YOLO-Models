...
**After training:** best weights land under the run folder (e.g. `runs/detect/train/weights/best.pt`); zip/download from Colab and use them locally with **`yolo_detect.py`** or the **[candy calorie counter](examples/candy_calorie_counter)** example, which maps detected classes to nutrition info—another way to use “sorted” per-class results.
