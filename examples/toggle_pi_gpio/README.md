...
Software required:
* Before running this example, set up the Raspberry Pi to run YOLO models.
...
## Software Setup
Once Raspberry pi setup is completed, move into the `~/yolo` folder and activate the `venv` environment by issuing: 
```
cd ~/yolo
...
```
wget https://raw.githubusercontent.com/SyedAliRazaGilani/Train-and-Deploy-YOLO-Models/refs/heads/main/examples/toggle_pi_gpio/toggle_pi_gpio.py
```
